#!/usr/bin/env node
// create-dsh-content — scaffold a content-automation DSH plugin. Zero dependencies.
...
async function main() {
  ...
  let cfg = {
    targetDir,
    name: flags.name,
    template: flags.template || 'content',
    pluginId: flags['plugin-id'],
    toolName: flags['tool-name'],
    platforms: flags.platforms || 'devto,bluesky',
    withCI: flags['with-ci'] !== false,
    verify: flags.verify === true ? true : undefined,
    skipInstall: flags['skip-install'] === true,
  }

  const needWizard = !cfg.targetDir
  ...
  cfg.template = cfg.template && TEMPLATES.includes(cfg.template) ? cfg.template : 'content'
  if (!TEMPLATES.includes(cfg.template)) {
    console.error(err(`✘ 未知模板 / unknown template: ${cfg.template} (choose ${TEMPLATES.join('|')})`))
    process.exit(1)
  }

  // Fill remaining fields with safe defaults.
